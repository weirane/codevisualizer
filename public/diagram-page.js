...
async function handleNodeSelect(node) {
  ...
  try {
    ...
    // Render per-file exports usage list when a file node is selected
    if (node.type === "file" && filePath) {
      renderExportsUsageList(filePath);
    } else {
      clearExportsUsageList();
    }
    ...
    const clones =
      (diagramData &&
        diagramData.clones &&
        node.id &&
        diagramData.clones[node.id]) ||
      [];
    await renderClonePreviews(clones);
  } catch (error) {
    console.error("Error in handleNodeSelect:", error);
    setDetailMessage(
      "Error loading node details. Check console for more information."
    );
  }
}
...
async function renderClonePreviews(clones) {
  ...
  for (const clone of limited) {
    ...
    try {
      const payload = await fetchSourceSnippet(
        lastReport.rootPath,
        clone.filePath
      );
      const snippet = extractCloneSnippet(
        payload.content,
        clone.startLine,
        clone.endLine
      );
...
function showCloneOverlay(title, nodes) {
  ...
  if (!nodes || nodes.length === 0) {
    ...
    nodes.forEach((c) => {
      ...
      path.textContent = `${c.filePath} (${Math.round(
        (c.similarity || 0) * 100
      )}% match)`;
...
async function handleFindClones(node) {
  ...
  const nodeIndex =
    diagramData && diagramData.index
      ? diagramData.index instanceof Map
        ? diagramData.index
        : new Map(Object.entries(diagramData.index))
      : new Map();
  ...
  const aggregated = [...byFile.values()].sort(
  ...
  for (const agg of aggregated) {
    ...
    try {
      ...
      best.snippet = extractCloneSnippet(
        payload.content,
        best.startLine,
        best.endLine
      );
...
function ensureExportsContainer() {
  if (exportsContainer && exportsContainer.parentElement) {
    return exportsContainer;
  }
  exportsContainer = document.createElement("div");
  exportsContainer.className = "diagram-detail__exports";
  const header = document.createElement("h4");
  header.textContent = "Exports in file";
  header.style.margin = "0.5rem 0";
  const list = document.createElement("div");
  list.className = "diagram-detail__exports-list";
  exportsContainer.append(header, list);
  // insert after meta, before code
  const body = detailPanel.querySelector(".diagram-detail__body");
  if (body) {
    body.insertBefore(exportsContainer, detailCode);
  }
  return exportsContainer;
}

function clearExportsUsageList() {
  if (!exportsContainer) return;
  const list = exportsContainer.querySelector(".diagram-detail__exports-list");
  if (list) list.innerHTML = "";
}

function renderExportsUsageList(filePath) {
  const container = ensureExportsContainer();
  const list = container.querySelector(".diagram-detail__exports-list");
  if (!list) return;
  list.innerHTML = "";

  const sg = lastReport && lastReport.structureGraph;
  const exported = (sg && sg.exports && sg.exports[filePath]) || [];
  const usageMap = (sg && sg.exportUsage) || {};

  if (!exported.length) {
    const p = document.createElement("p");
    p.className = "help-text";
    p.textContent = "No exports detected in this file.";
    list.appendChild(p);
    return;
  }
  ...
  const rows = exported
    .map((name) => ({
      name,
      count: Number(usageMap[`${filePath}#${name}`] || 0),
    }))
    .sort((a, b) => a.name.localeCompare(b.name));

  // Build an index from name to symbol id for click-to-select when available
  const index =
    diagramData && diagramData.index
      ? diagramData.index instanceof Map
        ? diagramData.index
  ...
  index.forEach((n) => {
    if (!n || !n.name || n.path !== filePath) return;
    nameToId.set(n.name, n.id);
  });

  rows.forEach((ex) => {
    const row = document.createElement("div");
    row.className = "diagram-detail__export-row";
    row.style.display = "flex";
    row.style.justifyContent = "space-between";
    row.style.gap = "0.5rem";

    const maybeId = nameToId.get(ex.name);
    let left;
    if (maybeId) {
      ...
      btn.title = "Focus this export in the diagram";
      btn.addEventListener("click", () => selectDiagramNode(maybeId));
      left = btn;
    } else {
    ...
    countEl.textContent = `${ex.count} importer${ex.count === 1 ? "" : "s"}`;
...
function selectDiagramNode(nodeId) {
  if (!nodeId || !controller || typeof controller.setSelected !== "function")
    return;
  controller.setSelected(nodeId);
  // Also update detail panel to reflect the symbol selection
  const index =
    diagramData && diagramData.index
      ? diagramData.index instanceof Map
        ? diagramData.index
